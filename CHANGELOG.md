...
- **接地词汇刷新（09-22,作者两问："只对着我的例子调?""是否该用 LLM:先搜相关内容再让它想会涉及哪些词"）**：承认前两天的版本词/后继探测只覆盖产品线形状,手动补别名是一次性修补。通用机制 `services/vocab_refresh.py`：每目标每天一次,把它自己近 14 天线报的**标题**（最多 80 条）交给规划模型,让它挑"认真跟这个目标的人会按名字盯的东西"（版本/代号/药物/试验/对手/活动/人/组织/地点）。**模型提案,数据裁决**：词必须逐字出现在给它的标题里、≥2 条支持、不是已有别名或公共词、每轮 ≤6;**别的目标的别名一律拒绝**,同一词被多目标提出时归支持数最高者（对比稿里的对手产品）;**版本/代号/产品/药物/试验/活动自动成别名,人/组织/地点只建议**（雷达页一次点击——自动加人会把"大谷"目标悄悄变成"道奇队"目标;目标本身的名字别名不受影响,照常搜索）。干跑实证：渐冻症学到 GNK-301 / ACE-2223 / COYA 302（药物代号）、ALSTARS（试验）、TDP-43/TREM2、Walk to Defeat ALS;openAI 学到 GPT-6 / Astra（代号）;大谷 → 主帅/队友/Padres 进建议区。**纪律**：先通用机制,后例子验证。
- 测试 70 → 103。
