...
- **架构收口而非补丁（09-06,作者要求"不要打补丁"）**：上面①的病根是"原则未被结构强制"——消费期仍能从 URL 推导一手、生命周期规则内联四处、"目标是什么"三处各拼一份。收口：(1) 迁移 0020 把 1,256 条 NULL 盖章旧行一次性盖上,此后盖章恒有值,`semantic_ingest`/`publish_service` 的 URL 兜底代码全部删除——`is_first_party` 只剩入口一个调用方;(2) `services/lifecycle.py::lifecycle_for` 成为唯一生命周期规则（出生/加入/盖章升级/迁移都调它,运行中只升不降）;(3) `services/target_profile.py::TargetProfile` 成为唯一目标定义（`terms()` 给相关性门、`describe()` 给摘要模型、`matcher()` 给跨目标可见性）。测试 89→91。
- **内置默认模型切到 gemini-3.8-flash**（作者要求;实机验证模型 id 可用）。
- **消防栓源的层级（09-07 两天回测发现）**：装机后 96 条提炼里 79% 被模型判无关、83 条"已证实"中 81 条单源——78 篇是 **arXiv 类目 feed**（cs.AI/LG/CL/CV）。根因是分层语义混淆："谁发布的"（arXiv 是论文的一手）≠"是不是目标自己的频道"（CONFIRMED 的含义）；类目源是 firehose 却走 CURATED 路由，再被 arxiv.org 地板抬成 primary → 出生即已证实 → 融合门放行。架构修法：`provenance.is_firehose_url` + preset `source_type=arxiv` → 路由恒为 AGGREGATED（垃圾地板/keep_keywords/佐证都生效,永不 CONFIRMED）；建议源里的 arXiv 查询 feed 同理；arxiv.org 移出全局一手地板。迁移 0021 重盖存量 arXiv 条目（副本实测 1,621 条）并降级仅靠它们确认的线索（1,526 条,摘要保留）。连渐冻症目标此前都在收 cs.AI 论文（academic_research 集合）,现在被 keep_keywords 在入口挡下。
- **没有裁判就不并线（09-09,作者截图：8/13 的"Claude Code 自动模式默认"线索被"配额+25%"和 36Kr 文化稿翻出来标"进展 刚刚"）**：两条误并成员与线索的中心相似度只有 0.60/0.56（灰区）,却没有仲裁记录——灰区合并在**仲裁预算耗尽**或**仲裁调用出错**时退回"按嵌入合并"。方向错了：错并会污染摘要并把旧线索伪装成进展（不可逆）,而拆分可由全局池/故事线事后修复（可逆）。改为：预算耗尽或调用出错 → 不并,新建线索;纯 RSS 无生成模型的嵌入合并地板保留。数据修复：#6260 剥离两条误并（11 成员/11 家出版方,"进展"退回 8/18）;用仲裁复核近 7 天被重摘要且有 14 天后加入成员的 37 条线索,180 次调用,135 条晚到成员确认同事件保留,**45 条剥离**（16 条不同事件、29 条同故事线不同事件）,22 条线索重算计数与时间戳,剥离条目下一轮重新聚类。教训：审核脚本长事务与运行中的应用抢锁,丢了约 95 条 token 记账（下次按线索逐条提交）。
- **融合按透镜判相关性（09-09,作者截图：DeepMind 天气模型经 openAI 目标抓到,被摘要模型判"与监测目标 OpenAI 无关"）**：全局线索让线索跨目标唯一、透镜已含 gemini/grok/openAI,但融合仍站在"哪个目标的处理轮次碰到它"的立场,只给摘要模型那一个目标的画像。改为 `_lens_profile`：把透镜内全部目标（owner ∪ tracker_ids ∪ 成员 owner ∪ 成员可见性）的画像逐行交给摘要模型,相关性规则改为"至少一个列出目标参与事件即相关,只有一个都不沾才是 NOISE,并在摘要里点名涉及哪些目标"。板块/用户指令仍取自本轮目标。这是透镜设计欠的最后半步。
- **目标即查询（09-17,作者"是不是有更简单有效的解法"→"开始吧"）**：三周的跨目标缺陷是同一个缺陷——内容被抓到它的目标"拥有"。用一个关系 `ThreadTarget(thread, target, source, llm_verdict)` 替换全部补偿机制：文章/线索全局无主;关系由对称的确定性匹配（对所有目标一视同仁）+ 聚合层的发现路由产生;融合按线索走一遍、摘要中立只写事件,"涉及哪些目标"是独立的结构化输出写回 `llm_verdict`（False 只折叠不删除）;新规则"一个目标都不涉及的线索不花钱"。删除：`also_tracker_ids` / `tracker_ids` 透镜的全部读写、入库可见性盖章、按目标融合轮次、`_lens_profile`、提示词相关性规则。迁移 0022 从成员重建关系（24 秒,21,966 条,1,830 条线索无人关心,3,641 条涉及 ≥2 目标）。真模型冒烟：WeatherNext → 中立摘要 + gemini=True（旧透镜还挂着 grok/openAI）;"Claude's Theorem"论文 → 中立摘要 + claude=False（折叠可核对——模型仍判同名撞车,架构让这类错判代价小且可见）。设计记录 `docs/targets_are_queries.md`。
- **目标即查询 8 小时回测 + 仲裁关思考（09-17 晚）**：83 轮、314 新文章、243 新线索。提炼 8 条,模型判 NOISE **1 条（12%,上次 79%）**;"无目标关心不花钱"拦下 35 条、门控 207 条、无实质增量 11 条,实际融合仅 7 次;新摘要含"无关/监测目标"措辞 0 条（中立性成立）;模型判定 9 真 3 假,3 条假均合理（OpenAI 的失准报告框架提到 Claude/Grok 但二者非参与方;blog.google 的 Data Commons 帖与 Gemini 无关）;不变量四项全 0;仲裁 0 次预算耗尽。**新发现：融合降下来后,事件仲裁成了生成支出的 92%（777 次 / 22.3 万 token）,其中 52% 是一字答案背后的隐性思考 token。** `GeminiProvider.generate(thinking_budget= / thinking_level=)`（模型不支持则静默回退）。作者裁"降等级不关闭":9 对×2 实测 默认 16/18 正确 348 token/次,**low 17/18 / 177**,关闭 17/18 / 189——仲裁用 `thinking_level="low"`,更便宜也更稳。
- **Gemini 4 Pro 泄露一条没抓到（09-18,作者报）**：库里其实有 4 条（HN/reddit/DoNews）,但全是聚合层单条、彼此没被认亲（仲裁只比最近 3 个候选,全被 3.8 Flash 大簇占住）,躺在折叠区;nokiapoweruser/officechai/X 的主力报道一条没进。三层原因与三层修法：① X-first 泄露→通道仍黑（等作者 xAI key/小号）;② **笼统查询淹没具体词**——实测 gnews "gemini" 100 条里只有 2 条提 Gemini 4,"Gemini 4 Pro" 单查即命中主力报道;而 resolver 把用户关键词已覆盖的英文版本里的别名一律跳过（视为"已覆盖"）。改为**具体别名各自成路由**（`gnews_alias_N`,版本词优先,每目标上限 6;OR 合并实测把 10 条稀释成 5 条,故不合并）。③ "什么在路上"不能问规划器——实测它给 Gemini 的"下一代"是 Gemini 2.5 / Gemini 3（世界知识停在训练截止）。**涌现关键词**（P4.2 扩展,`extract_version_terms`）：从目标线报**标题**里抽别名锚定的版本短语（"Gemini 4 Pro"）,跨 ≥3 条线索反复出现且不在别名里 → 雷达页提示"线报里反复出现 X,要不要加进追踪词",接受即成别名（匹配器与路由随之派生）。同时 replan 的别名合并改为**只增不删**。gemini 目标已手动加入 "Gemini 4" / "Gemini 4 Pro"（删掉规划器给的过期版本词）。
- **仲裁基准脚本** `scripts/bench_arbiter.py`：对任意 provider（含 OpenAI 兼容端点）跑标注对,报并线决策正确率、故事线召回、token/次、延迟——作者拿到某新模型测试权限后可直接评估它是否适合做仲裁（key 走设置/环境变量,不进命令行）。
- **自然新版本全自动（09-18,作者："如果用户建任务要加一堆别名就很蠢…实体查询就是为了让意图被充分转化"）**：两处补齐。① resolver 对每个带版本号的别名自动探测**后继版本**（最高已知版本的下一个小版本与下一个大版本："Gemini 4" → "Gemini 4.1"、"Gemini 5"），零 token 条件 GET,名字出现前什么都不回;别名路由上限 6→8。② 涌现关键词**不再要用户点"加入"**：锚定目标自身名字、版本 ≥ 已知、跨 ≥3 线索反复出现的版本短语在扫描时直接成为别名（日志记录,雷达不再提问）;账号/出版方建议仍需确认（跟一个人是判断题）。闭环：探测后继名 → 名字出现即被抓 → 反复出现即成正式别名 → 匹配器与路由随之派生——全程不问用户,也不依赖规划器的过期知识。
- **接地词汇刷新（09-22,作者两问："只对着我的例子调?""是否该用 LLM:先搜相关内容再让它想会涉及哪些词"）**：承认前两天的版本词/后继探测只覆盖产品线形状,手动补别名是一次性修补。通用机制 `services/vocab_refresh.py`：每目标每天一次,把它自己近 14 天线报的**标题**（最多 80 条）交给规划模型,让它挑"认真跟这个目标的人会按名字盯的东西"（版本/代号/药物/试验/对手/活动/人/组织/地点）。**模型提案,数据裁决**：词必须逐字出现在给它的标题里、≥2 条支持、不是已有别名或公共词、每轮 ≤6;**别的目标的别名一律拒绝**,同一词被多目标提出时归支持数最高者（对比稿里的对手产品）;自动还是建议**由模型逐词判断**（作者:"让 LLM 自己判断"）：`scope=core`（目标自己的故事——它的版本、为它做的药/试验、它的官方活动,按此名搜只会带来目标的新闻）自动成别名;`scope=context`（相关但按此名搜会带来不是目标的新闻——对手、队友、场馆）只建议（雷达页一次点击）。按类型硬分两边都错：赞助药企对疾病目标是核心,队友对球员目标是外延。目标本身的名字别名不受影响,人/地点类目标照常搜索。干跑实证：渐冻症学到 GNK-301 / ACE-2223 / COYA 302（药物代号）、ALSTARS（试验）、TDP-43/TREM2、Walk to Defeat ALS;openAI 学到 GPT-6 / Astra（代号）;大谷 → 主帅/队友/Padres 进建议区。**纪律**：先通用机制,后例子验证。
- **事后合并（09-22,作者截图：Opus 5.5 发布出了两张卡）**：根因是仲裁只问最近 3 个候选——《Opus 5.5 matches **Fable 5.1**…》最近的 3 个候选全是 Fable 发布线索,3 分钟前出生的 Opus 5.5 线索排第 4 没被问到,判"都不是"→ 新线索;后续泄露稿被救援进第二条。入库"宁拆勿错并"是对的（可逆）,缺的是**可逆的另一半**：`services/thread_merge.py` 每轮语义任务后,对近 48h 内中心相似度 ≥0.70 的线索对逐对问同一个仲裁,判 event 即合并进较早的线索（成员/目标关系/告警/故事线并入,计数与中心重算,摘要保留并作为实质增量触发重摘要）;其他判定存 `ThreadPairVerdict` 不再重复问;每轮 ≤20 对。逐对询问没有候选数盲区。
- **合并策略收成一处 + 事后遍认亲（09-22,作者"是架构还是补丁"）**：`services/merge_policy.py` 是"何时判两件事为同一事件"的唯一声明（入库:候选地板 0.05 / 置信线 0.80 / top-3 / 每轮 300 次;事后:相似 ≥0.70 / 48h / 每轮 20 对,含校准依据）,`semantic.py`/`semantic_ingest.py`/`thread_merge.py` 都从它取值;事后遍判"同故事线"时按入库同一出生规则认亲,不再只记不用。
- **提炼卡"内容和标题乱窜"（09-22,作者报;即 P6 搬迁时退化、8/13 挂账的"引用 vs 佐证"）**：卡片的"N 处溯源"一直列的是**最新 8 个成员**,不是摘要引用的来源——Gemini 4 Pro 的卡下面挂着一篇 Anthropic 稿,就是这个。修：融合时把引用成员 id 存进 `StoryThread.cited_article_ids`（迁移 0023 从摘要附录的链接回填存量）;API 先列引用（标 `cited`）再列同事件佐证;卡片展开分"摘要引用 / 同事件佐证"两组,标题链接指向引用来源。线索成员本身核查过是干净的（Opus 5.5 线索 33 条全是该事件）。
- **学出来的关系（09-24,作者"分类好多次都有问题,终极方案是什么"）**：终极方案=不再写规则,让"线索是否关于目标"从雷达自己的标签里学。`services/relation_model.py`：每目标一个线性探针（去均值中心向量上的 L2 逻辑回归,numpy）,标签=摘要模型的参与判定 + 官方域名条目（高精度正例）+ 判给对手目标的线索与无关系的背景线索（负例）;每日维护重训,5 折 AUC ≥0.90 且正负各 ≥20 才启用,否则匹配器仍是地板（新目标从地板起）。应用只做两件事、都只在高置信：补匹配器漏掉的关系（p ≥0.80）,否决匹配器/路由的误关联（p ≤0.20,否决=折叠不删）。离线：AI 四目标 AUC 0.979–0.996;"Introducing GPT-6 Sol" openAI 0.89 / claude 0.25 / gemini 0.20,**没有任何一条关于"比较"的规则**。干跑近 7 天 3472 条：补 0、否决 719,抽样全对。第一版校准错误当场修（负例只有对手线索时,VS Code 发布说明被 0.4 补给三个目标——加入背景负例、门槛钉死）。迁移 0024（`ThreadTarget.score`）。
- **"被比较 ≠ 参与"（09-24,作者"分类还是有问题"）**：近 24h 45 条提炼里错的是一类——《Introducing GPT-6 Sol and Luna》挂了 openAI/claude/gemini 三个目标：发布稿拿对手做基准,匹配器按"正文提到 ≥2 次"关联,摘要模型又被提示"拿不准就算相关"。两层收紧：匹配器的正文规则改为**≥2 次且至少一次在导语（前 600 字）**;摘要模型的 `concerned_targets` 规则明写"被比较/被基准/被引为对手/被列为替代/顺带提及都不算参与,拿不准就不算;但同一主题的多语言/地区目标一视同仁"。一次性：按新规则剪掉近 30 天匹配器独有的关系 965 条,用专用判定重判近 7 天 54 条多目标提炼线索（42 处改判,GPT-6 Sol / Meta Muse 等归位）;重判把渐冻症四语言目标当四个实体误摘,按"同主题语言变体"规则复原。
- **TrendScan 并入 alert_engine（09-24,作者裁决）**：独立的趋势扫描任务删除;`alert_engine.evaluate_entity_spikes` 在每轮语义任务后做同一件事（多条提炼线索的摘要点名同一实体 = 跨线索共振）,但更紧：≥3 条线索（原 2）、每实体每天一次（原 12h）、每轮最多 3 次生成（原不封顶,实测 39 次/30 万 token/日）、low 思考。仍写 TrendAlert,仪表盘不变。
- **回测（09-24）**：合并遍 233 对判定 → 20 合并（抽查全对）/ 76 认亲 / 137 不同;提炼 78 条 NOISE 1;无目标拦截 67;词汇自动学到 5 个新名（含药名 Ulefnersen、FUS-ALS）;不变量全 0;仲裁 194 token/次。TrendScan 成本上涨为新观察项（见路线图）。
- 测试 70 → 107。
