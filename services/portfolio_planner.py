...
_INTENT_SYSTEM = (
    ...
    "narration_lang and must never widen or narrow search coverage. For a PRODUCT or MODEL "
    "line, also include the names the community uses for what is COMING — the successor "
    "version ('Gemini 4', 'GPT-6'), known internal codenames, and the pending version people "
    "are waiting on — because leaks and pre-release tests are reported under those names, "
    "weeks before launch, and a generic query drowns them (role='product').\n"
