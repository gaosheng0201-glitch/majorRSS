...
def process_tracker_fusion(tracker_id: int):
    ...
    while True:
        ...
        db.set_pipeline_status(tracker.name, "AI Fusion", f"Gemini is cross-validating a batch of {len(unprocessed)} sources (Full Text)...")
        ...
        if hasattr(result, 'duplicate_of_report_id') and result.duplicate_of_report_id is not None:
            ...
            if merged:
                print(f"Deduplication: Merged {len(unprocessed)} articles into existing IntelReport {result.duplicate_of_report_id}")
