...
> 创建：2026-07-23 · **状态：§1–§6 已全部实现（2026-07-23 清债），本文保留为设计记录**
>
> 收口进度：§1 计费公式+embedding记账 ✅（`services/pricing.py`，输入/输出分算，真实模型名，官方价格）；
> §2/§3 模型配置 UI + 本地嵌入 ✅（Settings「模型与后端」+ `/settings/llm-config`）；
> §4 向量坍缩+事件聚类 ✅（去均值 + LLM 事件仲裁 + 候选地板）；§5 多语言归并 ✅（跨语言同事件合并已验证）；
> §6 Gemini Embedding 2 ✅。此外路由分组 ✅（组间全抓/组内首成即停）。
>
