...
## 关系来源的第四层（2026-09-24）：学出来的关系

`services/relation_model.py`——每目标一个线性探针,从雷达自己的标签学（摘要判定 / 官方域名正例 / 对手线索与背景线索负例）,每日重训,AUC ≥0.90 才启用。它对确定性匹配器只做两件高置信的事：补漏（p ≥0.80,`source=model`）、否决误关联（p ≤0.20,`llm_verdict=False`,折叠不删,`score` 存分数）。匹配器退为冷启动地板。这是三周"再加一条字符串规则"的替代物：规则认形状,探针认向量空间里的可分性;标签每天自动增长,用户的一次纠错（P3.1）将是最高等级的标签。
